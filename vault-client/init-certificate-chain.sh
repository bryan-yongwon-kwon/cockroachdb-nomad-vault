...
echo generating new certificate for roach0
vault write -format=json pki_int/issue/example-dot-com common_name="node" alt_names="roach0.crdb.io" ip_sans="::" ttl="24h" > /cockroach-certs/vault_response.json
mkdir -p /cockroach-certs/roach0/
cat /cockroach-certs/vault_response.json | jq -r .data.issuing_ca > /cockroach-certs/roach0/ca.crt
cat /cockroach-certs/vault_response.json | jq -r .data.certificate > /cockroach-certs/roach0/node.crt
cat /cockroach-certs/vault_response.json | jq -r .data.private_key > /cockroach-certs/roach0/node.key
chmod 0600 /cockroach-certs/roach0/node.key

echo generating new certificate for roach1
vault write -format=json pki_int/issue/example-dot-com common_name="node" alt_names="roach1.crdb.io" ip_sans="::" ttl="24h" > /cockroach-certs/vault_response.json
...
vault write -format=json pki_int/issue/example-dot-com common_name="node" alt_names="roach2.crdb.io" ip_sans="::" ttl="24h" > /cockroach-certs/vault_response.json
